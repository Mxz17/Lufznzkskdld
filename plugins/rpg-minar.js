...
conn.reply(m.chat, `> ∙۬◌⃘࣭ٜ࣪࣪࣪۬፝͜🤍᪶۫۫྆•≭ Hola ${nombre}, Ya has minado recientemente, espera ⏱️ *${tiempo2}* para regresar a la Mina.`, m, rcanal)
return
}
global.db.data.users[m.sender].exp += dinero
let minar = `> ∙۬◌⃘࣭ٜ࣪࣪࣪۬፝͜🤍᪶۫۫྆•≭ Genial! minaste *${dinero} 💫 XP.*`
await m.react('⛏️')
...
handler.help = ['minar']
handler.tags = ['rpg']
handler.command = ['minarc', 'miming', 'mine'] 
...
conn.fakeReply(m.chat, info, '0@s.whatsapp.net', '> ∙۬◌⃘࣭ٜ࣪࣪࣪۬፝͜🤍᪶۫۫྆•≭ *Minando.. - GenesisBot* 💣', 'status@broadcast' )
await m.react('⛏️')
