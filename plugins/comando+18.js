...
const handler = async (m, {conn, command, usedPrefix}) => {
  if (!db.data.chats[m.chat].modohorny && m.isGroup) throw '> ∙۬◌⃘࣭ٜ࣪࣪࣪۬፝͜🍿᪶۫۫྆•≭ *¡Estos comandos están desactivados!*';
conn.reply(m.chat, `> ∙۬◌⃘࣭ٜ࣪࣪࣪۬፝͜🍿᪶۫۫྆•≭ Enviando el *${command}*`, m, {
...
sourceUrl: canal }}})
   switch (command) {
    case 'pack':
      ...
      conn.sendMessage(m.chat, {image: {url: url}, caption: `_᠂⸱ེ̀.🥵 Packᮬ᭄_`}, {quoted: m});
      break;
    case 'pack2':
      const url2 = await packgirl[Math.floor(Math.random() * packgirl.length)];
      conn.sendMessage(m.chat, {image: {url: url2}, caption: `_᠂⸱ེ̀.🥵 Pack 2 ↺_`}, {quoted: m});
      break;
    case 'pack3':
      const url3 = await packmen[Math.floor(Math.random() * packmen.length)];
      conn.sendMessage(m.chat, {image: {url: url3}, caption: `_᠂⸱ེ̀.🥵 Pack 3 ❁_`}, {quoted: m});
      break;
    case 'videoxxx': case 'vídeoxxx':
      const url4 = await videosxxxc[Math.floor(Math.random() * videosxxxc.length)];
      await conn.sendMessage(m.chat, {video: {url: url4}, caption: `*Disfruta del video Papu ﭢ*`}, {quoted: m});
      break;
    case 'videoxxxlesbi': case 'videolesbixxx': case 'pornolesbivid': case 'pornolesbianavid': case 'pornolesbiv': case 'pornolesbianav': case 'pornolesv':
      ...
      await conn.sendMessage(m.chat, {video: {url: url5}, caption: `*Disfruta del video Papu ﭢ*`}, {quoted: m});
      break;
  }
};
