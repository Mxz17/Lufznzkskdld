...
let handler = async function (m, { conn, text, usedPrefix, command }) {
  ...
  if (name.length >= 100) return m.reply('> ∙۬◌⃘࣭ٜ࣪࣪࣪۬፝͜🍿᪶۫۫྆•≭ *El nombre es demasiado largo.*' )
  age = parseInt(age)
  if (age > 100) return m.reply('👴🏻 Wow el abuelo quiere jugar al bot.')
  if (age < 5) return m.reply('🚼  hay un bebé jsjsj. ')
  user.name = name.trim()
  user.age = age
  user.regTime = + new Date
  user.registered = true
  let sn = createHash('md5').update(m.sender).digest('hex').slice(0, 6)        
  let img = await (await fetch(`https://i.ibb.co/pWxKL1T/file.jpg`)).buffer()
