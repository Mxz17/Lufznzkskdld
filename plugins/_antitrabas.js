...
export async function before(m, {conn, isAdmin, isBotAdmin, usedPrefix}) {
  ...
  if (chat.antiTraba && m.text.length > 5000) { // Cantidad máxima de caracteres aceptados en un mensaje//
    ...
    conn.sendMessage(m.chat, `> ∙۬◌⃘࣭ٜ࣪࣪࣪۬፝͜🍿᪶۫۫྆•≭ *Se detecto un mensaje que contiene muchos caracteres*\n`, `${isBotAdmin ? '' : '*No soy administrador, no puedo hacer nada ._.*'}`, m);
    ...
    } else if (!bot.restrict) return m.reply('> ∙۬◌⃘࣭ٜ࣪࣪࣪۬፝͜🍿᪶۫۫྆•≭ *Para realizar acciones de eliminación, mi dueño tiene que encender el modo restringido!*');
  }
  return !0;
}
